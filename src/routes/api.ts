...
export function registerApiRoutes(app: Hono<AppEnv>) {
  ...
  app.post('/api/:pubId/upload', async (c) => {
    ...
    if (!pub || pub.owner_user_id !== user.id) return c.json({ error: 'Zabranjeno' }, 403);
    ...
    if (!kind) {
      return c.json({ error: 'Nepodržan tip. Slike: JPEG/PNG/GIF/WebP. Video: MP4/WebM.' }, 415);
    }
